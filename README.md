...
* 在 Session 2 和 Session 3的标注中，Bot主动提及历史信息的方式包含2种：
&ensp bot在**Session中间**主动提及(user首句开始，bot在聊天中主动提及) 和 bot在**Session首句**主动提及(bot首句开始就主动提及记忆)两种方式。
&ensp 标注时主要标注bot在**Session中间**主动提及(占绝大多数)；同时如果bot在**Session首句**主动提及记忆内容时，需要第一句话提及记忆时话术要比较自然，不能太突兀(毕竟是开启新session)。
